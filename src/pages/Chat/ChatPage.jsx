...
function SongCard({ song, isAI }) {
  ...
  const isCurrentPlaying =
    playingItem?.youtubeId === song.youtubeId && isPlaying;
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span
          className={cn(
            "block font-ui text-xs font-semibold truncate",
            isAI ? "text-text-main" : "text-white",
          )}
        >
          {song.title}
        ...
        <span
          className={cn(
            "block font-ui text-[10px]",
            isAI ? "text-text-sub" : "text-white/60",
          )}
        >
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex flex-col", isAI ? "items-start" : "items-end")}>
        {isAI && (
          <span className="font-ui text-xs text-text-sub mb-1 ml-1">
            {MIA_NAME}
          </span>
        )}
...
export default function ChatPage() {
  ...
  const handleSaveRename = async (convId) => {
    ...
    if (
      trimmed &&
      trimmed !== conversations.find((c) => c.id === convId)?.title
    ) {
  ...
  const handleSend = async (text = input.trim()) => {
    ...
    try {
      if (isGuest) {
        ...
        const historyForApi = guestMessages.map((m) => ({
          role: m.role,
          content: m.text,
        }));
        const userMsg = {
        ...
        const aiMsg = {
          ...
          text:
            data?.reply ||
            "Xin lỗi, mình chưa nghe rõ. Bạn nói lại giúp mình nhé?",
          time: new Date(),
          songs: data?.songs || [],
        };
      ...
      setSendError(
        "Không gửi được tin nhắn. Bạn kiểm tra kết nối mạng và thử lại nhé.",
      );
    } finally {
      setIsTyping(false);
    }
  ...
  const handleClearChat = () => {
    if (isGuest) {
      if (
        window.confirm(
          "Xoá đoạn hội thoại này? Lịch sử chat khách không được lưu nên sẽ mất hẳn.",
        )
      ) {
        setGuestMessages([]);
      }
  ...
  const displayMessages =
    activeMessages.length === 0 ? [INITIAL_MESSAGE] : activeMessages;
        ...
        <div className="p-5 border-b border-primary/20">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-primary/10 rounded-2xl">
            ...
            <div>
              ...
              <p className="font-body text-sm text-text-main font-medium">
                {todayMoodObj?.label || "Bình thường"}
              </p>
            </div>
          </div>
        </div>
        ...
        {isGuest ? (
          ...
          <div className="p-4 flex-1 flex flex-col min-h-0">
            <div className="flex flex-col items-center text-center gap-2 py-8 px-4 bg-primary/5 rounded-2xl border border-primary/10">
              ...
              <p className="font-ui text-xs text-text-sub leading-relaxed">
                Cuộc trò chuyện này sẽ không được lưu lại. Đăng nhập để Dora nhớ
                lịch sử và xem lại bất cứ lúc nào.
              </p>
            </div>
          </div>
        ) : (
          <div className="p-4 flex-1 flex flex-col min-h-0">
            <div className="flex items-center justify-between mb-3">
              <p className="font-ui text-xs text-text-sub uppercase tracking-wider">
                Hội thoại
              </p>
              <button
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        {editingId === conv.id ? (
                          ...
                          <>
                            <p className="font-body text-sm text-text-main truncate">
          ...
          <div className="max-w-2xl mx-auto flex flex-col gap-3">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              <button
                onClick={() =>
                  handleSend(
                    "Gợi ý cho mình một bài nhạc phù hợp với tâm trạng nhé 🎵",
                  )
                }
              ...
              <button
                onClick={() =>
                  navigate("/journal", {
                    state: {
                      openWrite: true,
                      mood: CHAT_MOOD_TO_JOURNAL_MOOD[lastMood] || "",
                    },
                  })
                }
              ...
              >
                <Wind
                  size={13}
                  className={
                    lastMood === "stress" ? "text-white" : "text-primary"
                  }
                />{" "}
                Thở thư giãn
              </button>
              <button
                onClick={() =>
                  navigate(
                    lastMood
                      ? `/explore?mood=${CHAT_MOOD_TO_CONTENT_MOOD[lastMood] || lastMood}`
                      : "/explore",
                  )
                }
