        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="font-display text-2xl md:text-3xl text-text-main font-bold">
              Hướng dẫn Điểm & Thành tựu
            </h1>
            <p className="font-body text-xs text-text-sub">
              Giải thích các chỉ số trong mục Wellness Insight
            </p>
          </div>
        </div>
          ...
          {SECTIONS.map((s) => (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3
                  className={`font-display text-lg font-semibold ${s.textColor}`}
                >
                  {s.title}
                </h3>
              </div>
              ...
              <div className="flex flex-col gap-2.5">
                {s.body.map((p, i) => (
                  <p
                    key={i}
                    className={`font-body text-sm leading-relaxed ${s.textColor === "text-white" ? "text-white/95" : "text-text-sub"}`}
                  >
                    {p}
                  </p>
                ))}
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-bg/70">
                      <tr>
                        {s.table.headers.map((h) => (
                          <th
                            key={h}
                            className="font-ui text-[11px] text-text-sub uppercase tracking-wider px-4 py-2"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {s.table.rows.map((row, i) => (
                        <tr
                          key={i}
                          className={i % 2 === 0 ? "bg-white" : "bg-bg/30"}
                        >
                          {row.map((cell, j) => (
                            <td
                              key={j}
                              className="font-body text-sm text-text-main px-4 py-2"
                            >
                              {cell}
                            </td>
                          ))}
