          ...
          {leveledUp && (
            <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4">
              <p className="font-body text-sm text-text-main">
                ⭐ Bạn đã đạt{" "}
                <span className="font-bold text-primary">Level {newLevel}</span>
              </p>
            </div>
          )}
          {achievement && (
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const levelBefore = xp
      ? Math.floor(Math.max(0, xp.totalXp - xp.xpEarnedToday) / 100) + 1
      : 1;
  ...
  const totalMoodScore = moodLogs.reduce(
    (acc, m) => acc + (m.moodScore || 4),
    0,
  );
  const averageMoodScore =
    totalMoodEntries > 0 ? totalMoodScore / totalMoodEntries : 4.5;

  // Fallback: nếu chưa có mood logs, tính từ journals
  const totalEntries = journals.length;
  const journalScores = journals.reduce(
    (acc, j) => acc + (MOOD_SCORES[j.mood] || 4),
    0,
  );
  ...
  const getWeeklyMoodData = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayJournals = journals.filter(
        (j) => new Date(j.date).toDateString() === dStr,
      );
      if (dayJournals.length > 0) {
        const avgScore =
          dayJournals.reduce((s, j) => s + (MOOD_SCORES[j.mood] || 4), 0) /
          dayJournals.length;
            ...
            <div className="bg-[#FBF7EC] border border-[#E8E1CF] rounded-3xl p-6 shadow-sm">
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="bg-white/80 border border-[#F0EBDC] rounded-2xl p-4 shadow-inner">
                  <span className="font-ui text-xs text-text-sub uppercase tracking-wider block mb-1">
                    Điểm TB tuần này
                  </span>
                ...
                <div className="bg-white/80 border border-[#F0EBDC] rounded-2xl p-4 shadow-inner">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="font-display text-3xl text-secondary font-bold">
                      {totalEntries}
                    </span>
                    <span className="font-ui text-xs text-text-sub">
                      bản ghi
                    </span>
                  </div>
                  <p className="font-body text-xs text-text-sub mt-2">
                    Đã được mã hóa & lưu trữ an toàn
                  </p>
                </div>
                ...
                <div className="bg-white/80 border border-[#F0EBDC] rounded-2xl p-4 shadow-inner">
                  ...
                  <p className="font-body text-xs text-text-sub mt-2">
                    Duy trì thói quen giúp tâm lý ổn định
                  </p>
                </div>
                ...
                <div className="bg-white/80 border border-[#F0EBDC] rounded-2xl p-4 shadow-inner">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="font-display text-3xl text-text-main font-bold">
                      {totalMoodEntries}
                    </span>
                    <span className="font-ui text-xs text-text-sub">
                      lần ghi nhận
          ...
          <main className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="font-display text-2xl md:text-3xl text-text-main font-bold">
                    Wellness Insight
                  </h1>
                  <p className="font-body text-xs text-text-sub">
                    Phân tích biểu đồ và xu hướng cảm xúc
                  </p>
                </div>
              </div>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-primary to-[#123152] rounded-3xl p-6 shadow-md">
              <div className="absolute -right-6 -top-6 text-8xl opacity-10 select-none">
                💡
              </div>
              <h3 className="font-display text-lg text-white font-semibold mb-2 relative">
                💡 AI Insight hôm nay
              </h3>

              {dailyInsight?.available ? (
                <>
                  <p className="font-body text-sm text-white/95 leading-relaxed relative">
                    {dailyInsight.insight}
                  </p>
                  {dailyInsight.nextTip && (
                    <p className="font-ui text-xs text-white/75 mt-3 relative">
                      🌤️ Gợi ý cho ngày mai: {dailyInsight.nextTip}
                    </p>
                  )}
                </>
              ) : (
                <div className="relative">
                  <p className="font-body text-sm text-white/85 leading-relaxed">
                    Trò chuyện cùng Dora hôm nay để nhận nhận xét riêng dành cho
                    bạn — AI sẽ đọc lại đoạn chat và cho bạn biết hôm nay có gì
                    đặc biệt. 💬
                  </p>
                  <Link
                    to="/chat"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-2xl p-4 shadow-sm flex flex-col gap-2">
                <span className="font-ui text-[11px] text-text-sub uppercase tracking-wider">
                  ⭐ Level {dailyInsight?.xpInfo?.level ?? 1}
                </span>
                ...
                <span className="font-body text-xs text-text-sub">
                  {dailyInsight?.xpInfo?.xpEarnedToday
                    ? `+${dailyInsight.xpInfo.xpEarnedToday} XP hôm nay`
                    : `${dailyInsight?.xpInfo?.totalXp ?? 0} XP tích lũy`}
                </span>
              </div>
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-2xl p-4 shadow-sm flex flex-col gap-2">
                ...
                <span className="font-display text-2xl text-primary font-bold">
                  {dailyInsight?.streak?.currentStreak ?? 0} ngày
                </span>
                <span className="font-body text-xs text-text-sub leading-snug">
                  🐱{" "}
                  {dailyInsight?.petMessage ||
                    "Momo đang chờ nghe bạn kể chuyện hôm nay."}
                </span>
              </div>
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-2xl p-4 shadow-sm flex flex-col gap-2">
                ...
                <span className="font-display text-2xl text-secondary font-bold">
                  ...
                  {dailyInsight?.tree?.deltaToday ? (
                    <span className="font-ui text-xs text-secondary/70 font-normal ml-1">
                      +{dailyInsight.tree.deltaToday}% hôm nay
                    </span>
                  ) : null}
                </span>
                <span className="font-body text-xs text-text-sub">
                  {dailyInsight?.tree?.treeCount ?? 0} cây đã trưởng thành
                </span>
              </div>
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-2xl p-4 shadow-sm flex flex-col gap-2">
                ...
                {dailyInsight?.achievement?.unlockedToday ? (
                  <>
                    <span className="font-body font-semibold text-xs text-text-main">
                      🏅 {dailyInsight.achievement.title}
                    </span>
                    <span className="font-body text-[11px] text-text-sub leading-snug">
                      {dailyInsight.achievement.description}
                    </span>
                  </>
                ) : (
                  <span className="font-body text-xs text-text-sub leading-snug">
                    Chưa có thành tựu mới hôm nay — mỗi bước cố gắng đều được
                    ghi nhận.
                  </span>
                )}
              </div>
            ...
            <div className="bg-white border border-[#E8E1CF] rounded-3xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-display text-lg text-text-main font-medium">
                    Xu Hướng Cảm Xúc
                  </h3>
                  <p className="font-ui text-[11px] text-text-sub">
                    Phân rã các tầng năng lượng cảm xúc theo 7 ngày qua
                  </p>
                </div>
                <div className="flex gap-4 text-xs font-ui">
                  <span className="flex items-center gap-1.5">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#1E4E79]" />{" "}
                    Tích cực
                  </span>
                  <span className="flex items-center gap-1.5">
                  ...
                  <span className="flex items-center gap-1.5">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#F0E2B0]" />{" "}
                    Nhẹ nhõm
                  </span>
                </div>
              </div>
              ...
              <p className="text-center font-ui text-[11px] text-text-sub mt-4">
                7 days analytics
              </p>
            </div>
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                {moodHistory.length > 6 && (
                  ...
                  >
                    {showAllMoodHistory
                      ? "Thu gọn"
                      : `Xem tất cả (${moodHistory.length})`}
                  </button>
                )}
              </div>
              <p className="font-ui text-[11px] text-text-sub mb-4">
                30 ngày gần nhất — từng lần bạn ghi nhận cảm xúc.
              </p>
              ...
              {moodHistory.length > 0 ? (
                <div className="flex flex-col gap-2 max-h-72 overflow-y-auto pr-1">
                  {[...moodHistory]
                    ...
                    .map((m) => (
                      <div
                        key={m.id}
                        className="flex items-center gap-3 bg-bg/50 border border-[#F0EBDC] rounded-2xl p-3"
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {m.note && (
                            <p className="font-body text-[11px] text-text-sub leading-relaxed truncate mt-0.5">
                              {m.note}
                            </p>
                          )}
                        </div>
                ...
                <div className="text-center py-6">
                  ...
                  <p className="font-ui text-xs text-text-sub">
                    Chưa có lịch sử mood — hãy check-in cảm xúc để bắt đầu ghi
                    lại.
                  </p>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* 📅 Tổng kết tuần */}
              <div className="bg-white border border-[#E8E1CF] rounded-3xl p-6 shadow-sm">
                <h4 className="font-display text-base text-text-main font-semibold mb-1">
                  📅 Tổng kết tuần
                </h4>
                <p className="font-body text-xs text-text-sub leading-relaxed mb-4">
                  7 ngày gần nhất — nhìn lại chặng đường thay vì chỉ hôm nay.
                </p>

                {weeklySummary && weeklySummary.activeDays > 0 ? (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-bg/50 rounded-2xl p-3">
                      <span className="font-display text-xl text-primary font-bold block">
                        {weeklySummary.totalXpEarned}
                      </span>
                      <span className="font-ui text-[10px] text-text-sub uppercase tracking-wider">
                        XP kiếm được
                      </span>
                    </div>
                    <div className="bg-bg/50 rounded-2xl p-3">
                      <span className="font-display text-xl text-secondary font-bold block">
                        {weeklySummary.activeDays}/7
                      </span>
                      <span className="font-ui text-[10px] text-text-sub uppercase tracking-wider">
                        Ngày hoạt động
                      </span>
                    ...
                    <div className="bg-bg/50 rounded-2xl p-3">
                      <span className="font-display text-xl text-text-main font-bold block">
                        +{weeklySummary.treeGrowthTotal}%
                      </span>
                      <span className="font-ui text-[10px] text-text-sub uppercase tracking-wider">
                        Cây cảm xúc
                      </span>
                    ...
                    <div className="bg-bg/50 rounded-2xl p-3">
                      <span className="font-display text-xl text-primary font-bold block">
                        {weeklySummary.achievementsUnlocked}
                      </span>
                      <span className="font-ui text-[10px] text-text-sub uppercase tracking-wider">
                        Huy hiệu mới
                      </span>
                    ...
                    {weeklySummary.bestDayTitle && (
                      <div className="col-span-2 bg-primary/5 border border-primary/10 rounded-2xl p-3">
                        <span className="font-ui text-[10px] text-text-sub uppercase tracking-wider block mb-0.5">
                          Ngày nổi bật nhất
                        </span>
                        <span className="font-body font-semibold text-xs text-text-main">
                          ✨ {weeklySummary.bestDayTitle}
                  ...
                  <div className="text-center py-6">
                    <p className="text-2xl mb-1">📅</p>
                    <p className="font-ui text-xs text-text-sub">
                      Chưa có dữ liệu tuần này — trò chuyện cùng Dora để bắt đầu
                      nhé.
                    </p>
                  </div>
                )}
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-3xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-1">
                  <h4 className="font-display text-base text-text-main font-semibold">
                    🏅 Huy hiệu tháng này
                  </h4>
                  {achievements.length > 4 && (
                    ...
                    >
                      {showAllAchievements
                        ? "Thu gọn"
                        : `Xem tất cả (${achievements.length})`}
                    </button>
                  )}
                </div>
                <p className="font-body text-xs text-text-sub leading-relaxed mb-4">
                  Những cột mốc bạn vượt qua trong tháng — sang tháng mới sẽ bắt
                  đầu lại từ đầu.
                </p>
                ...
                {achievements.length > 0 ? (
                  <div className="flex flex-col gap-2.5 max-h-64 overflow-y-auto pr-1">
                    {(showAllAchievements
                      ...
                      <div
                        key={`${a.date}-${idx}`}
                        className="flex items-start gap-2.5 bg-[#FFF4D6]/60 border border-[#F0E2B0] rounded-2xl p-3"
                      >
                        <span className="text-lg shrink-0">🏅</span>
                        <div className="min-w-0">
                          <p className="font-body font-semibold text-xs text-text-main truncate">
                            {a.title}
                          </p>
                          <p className="font-body text-[11px] text-text-sub leading-relaxed">
                            {a.description}
                          </p>
                          <p className="font-ui text-[10px] text-text-sub/70 mt-0.5">
                  ...
                  <div className="text-center py-6">
                    <p className="text-2xl mb-1">🏅</p>
                    <p className="font-ui text-xs text-text-sub">
                      Chưa có huy hiệu nào — hãy tiếp tục trò chuyện mỗi ngày!
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-3xl p-6 shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <p className="font-body text-xs text-text-sub leading-relaxed">
                    Tần suất và mật độ ghi nhật ký cảm xúc cá nhân của bạn trong
                    tuần.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-6 mt-6">
                  ...
                  <div>
                    <span className="font-ui text-xs text-text-sub uppercase tracking-wider block">
                      Điểm tuần
                    </span>
              ...
              <div className="bg-white border border-[#E8E1CF] rounded-3xl p-6 shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <p className="font-body text-xs text-text-sub leading-relaxed">
                    Mức cân bằng cảm xúc dựa trên phản hồi của cuộc trò chuyện
                    và nhật ký gần đây.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-10 mt-6">
                  <div>
                    <span className="font-display text-3xl text-secondary font-bold block">
                      {totalMoodEntries > 0
                        ? Math.round(
                            (moodLogs.filter((m) => m.moodScore >= 5).length /
                              totalMoodEntries) *
                              100,
                          )
                        : totalEntries > 0
                  ...
                  <div>
                    <span className="font-display text-3xl text-primary font-bold block">
                      {formattedAvg}
                    </span>
                    <span className="font-ui text-[10px] text-text-sub uppercase tracking-wider">
                      Cân bằng tâm lý
              ...
              <div className="flex flex-col gap-3">
                {recentJournals.length > 0 ? (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="font-body font-medium text-xs text-text-main truncate">
                              {j.text}
                            </p>
                  ...
                  <div className="text-center py-6">
                    <p className="text-2xl mb-1">📖</p>
                    <p className="font-ui text-xs text-text-sub">
                      Chưa có nhật ký nào để phân tích.
                    </p>
                  </div>
                )}
              </div>
