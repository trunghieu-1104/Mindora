...
function BreathingExercise() {
  ...
  const start = () => {
    ...
    const cycle = () =>
      runPhase("inhale", 4, () =>
        runPhase("hold", 7, () => runPhase("exhale", 8, cycle)),
      );
    cycle();
  };
  ...
  return (
    <div className="card text-center py-12">
      <h3 className="font-display text-2xl text-text-main mb-2">
        Bài tập thở 4-7-8
      </h3>
      <p className="font-body text-text-sub text-sm mb-10">
      ...
      <p className="font-body text-text-sub text-sm">
        Kỹ thuật 4-7-8 giúp hệ thần kinh bình tĩnh lại
      </p>
    </div>
  );
}
...
export default function ExplorePage({ musicList = [], podcastList = [] }) {
  ...
  const [podcastMoodFilter, setPodcastMoodFilter] = useState(
    () => searchParams.get("mood") || "all",
  );
  ...
  useEffect(() => {
    ...
    if (tab && ["music", "breathing", "podcast"].includes(tab))
      setActiveTab(tab);
    if (mood) {
      setActiveMood(mood);
  ...
  useEffect(() => {
    ...
    api
      .get("/contents?contentType=music&size=200")
      .then((res) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    api
      .get("/contents?contentType=podcast&size=20")
      .then((res) => {
        if (cancelled) return;
  ...
  const filteredPodcasts = allPodcasts.filter((p) => {
    ...
    const matchMood =
      podcastMoodFilter === "all" || p.mood === podcastMoodFilter;
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 relative">
      <div className="mb-8">
        ...
        <p className="font-body text-text-sub">
          Nhạc, thiền định và bài tập thở cho tâm trạng của bạn
        </p>
      </div>
      ...
      {activeTab === "music" && (
        <div className="animate-fade-in">
          ...
          {loadingMusic && (
            <p className="font-ui text-sm text-text-sub mb-4">
              Đang tải nhạc...
            </p>
          )}

          {!loadingMusic &&
            sourceMusicList.length > 0 &&
            filtered.length === 0 && (
          ...
          >
            {filtered.map((m, i) => {
              const isCurrentPlaying =
                playingItem && playingItem.youtubeId === m.youtubeId;
              return (
                <motion.div
                ...
                >
                  <div>
                    ...
                    <h4 className="font-body font-bold text-text-main text-base mb-0.5">
                      {m.title}
                    </h4>
                    <p className="font-ui text-xs text-text-sub mb-4">
                      {m.artist}
                    </p>
                  </div>
                  <button
              ...
              <div className="text-center py-12">
                ...
                <p className="font-ui text-text-sub text-sm">
                  ...
                  {podcastSearch && (
                    <span>
                      {" "}
                      cho "<strong>{podcastSearch}</strong>"
                    </span>
                  )}
                </p>
                <button
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="font-ui text-xs text-text-sub line-clamp-2 mb-2">
                      {p.ep}
                    </p>
